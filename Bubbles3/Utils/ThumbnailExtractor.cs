...
namespace Bubbles3.Utils
{
    ...
    internal static class ThumbnailExtractor
    {
        ...
        public static BitmapSource FromImage(Image image, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC)
        {
            ...
            if (useWIC)
            {
                ...
                BitmapSource thumb = GetThumbnail(frameWpf, size, useEmbeddedThumbnails, useExifOrientation ? GetRotation(frameWpf) : 0);
                stream.Dispose();
                return thumb;
#else
                // .Net 2.0 fallback
        ...
        /// <param name="useWIC">true to use Windows Imaging Component; otherwise false.</param>
        /// <returns>The thumbnail image from the given image or null if an error occurs.</returns>
        public static BitmapSource FromStream(Stream stream, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC, bool disposeStream = true)
        {
            if (stream == null || size.Width <= 0 || size.Height <= 0)
                throw new ArgumentException();
            ...
            if (useWIC)
            {
                ...
                try
                {
                    ...
                    // if the file contains no embedded thumbnail. The image quality is only slightly worse.
                    //stream.Seek(0, SeekOrigin.Begin);
                    frameWpf = BitmapFrame.Create(stream, BitmapCreateOptions.IgnoreColorProfile, BitmapCacheOption.None);
        ...
        public static BitmapSource FromFile(string filename, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC)
        {
            ...
            if (useWIC)
            {
                ...
                try
                {
                    using (FileStream stream = File.Open(filename, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        ...
                        //BitmapSource b = GetThumbnail(frameWpf, size, useEmbeddedThumbnails, useExifOrientation ? GetRotation(frameWpf) : 0);
                        //if (filename == "E:\\pix\\plump.book\\ail001ASI_201907072.jpg")
                        //{
                        //    JpegBitmapEncoder encoder = new JpegBitmapEncoder();
                        //    BitmapFrame outputFrame = BitmapFrame.Create(b);
                        //    encoder.Frames.Add(outputFrame);
                        //    encoder.QualityLevel = 100;
                        //    using (var wstream = new FileStream("E:\\pix\\plump.book\\test.jpg", FileMode.Create))
                        //    {
        ...
        private static BitmapSource GetThumbnail(BitmapFrame bmp, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, int rotate)
        {
            ...
            if (bmp.Thumbnail != null)
            {
                try
                {
                    ...
                    if (bmp.Decoder == null || (bmp.Decoder.Preview == null && bmp.Decoder.Frames == null) || useEmbeddedThumbnails == UseEmbeddedThumbnails.Always)
                    {
                        // Take the thumbnail if nothing else is available or if ALWAYS
                        thumb = (ScaleDownRotateBitmap(sourceWpf, scale, rotate));
                    }
        ...
        private static BitmapSource FinalizeBitmapSource(BitmapSource source)
        {
